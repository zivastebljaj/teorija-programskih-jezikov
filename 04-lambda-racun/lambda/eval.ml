...
let rec step = function
  ...
  | S.Pair (v1, e2) when is_value v1 -> Pair (v1, step e2)
  | S.Pair (e1, e2) -> Pair (step e1, e2)
  | S.Cons (v, es) when is_value v -> Cons (v, step es)
  | S.Cons (e, es) -> Cons (step e, es)
  | S.Fst (S.Pair (v1, _)) -> v1
  | S.Fst e -> S.Fst (step e)
  | S.Snd (S.Pair (_, v2)) -> v2
  | S.Snd e -> S.Snd (step e)
  | S.Match (S.Nil, e1, _, _, _) -> e1
  | S.Match (S.Cons (v, vs), _, x, xs, e2) -> S.subst [(x, v); (xs, vs)] e2
